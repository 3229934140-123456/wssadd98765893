...
import { DoctorFunnel, ReceptionistFunnel } from '@/types';
...
const doctorStages = (funnel: DoctorFunnel) => [
  { label: '医嘱→预约', rate: funnel.appointmentRate, loss: 100 - funnel.appointmentRate, key: 'appointment' },
  { label: '预约→提醒', rate: funnel.reminderRate, loss: 100 - funnel.reminderRate, key: 'reminder' },
  { label: '提醒→到诊', rate: funnel.arrivalRate, loss: 100 - funnel.arrivalRate, key: 'arrival' },
];

const receptionistStages = (funnel: ReceptionistFunnel) => [
  { label: '分配→提醒', rate: funnel.reminderRate, loss: 100 - funnel.reminderRate, key: 'reminder' },
  { label: '提醒→响应', rate: funnel.responseRate, loss: 100 - funnel.responseRate, key: 'response' },
  { label: '响应→到诊', rate: funnel.arrivalRate, loss: 100 - funnel.arrivalRate, key: 'arrival' },
];

const diagnosisMap: Record<string, string> = {
  appointment: '建议加强复诊医嘱沟通质量',
  reminder: '建议前台加强提醒频次和方式',
  response: '建议前台加强提醒频次和方式',
  arrival: '建议了解患者爽约原因，灵活调整预约时间',
};

const AttributionView = ({ type, personName, funnel, onClose }: AttributionViewProps) => {
