...
import { X, TrendingDown, Calendar, MapPin, ChevronDown, ChevronUp } from 'lucide-react';
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, BarChart, Bar } from 'recharts';
import type { ClinicData, ClinicWeeklyTrend } from '@/types';
...
const ClinicDetailDrawer = ({ isOpen, onClose, clinic, weeklyTrendData }: ClinicDetailDrawerProps) => {
  ...
  const treatmentSummary = useMemo(() => {
    return TREATMENTS.map(treatment => {
      ...
      const avgNoShowRate = totalAppointments > 0 ? +((tData.reduce((sum, t) => sum + t.noShows, 0) / totalAppointments) * 100).toFixed(1) : 0;
      return {
        treatment,
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin px-6 pt-5 pb-20 space-y-6">
          <div className="content-card p-5">
            <div className="flex items-center justify-between mb-4">
              <h4 className="text-sm font-semibold text-neutral-800">预约与到诊趋势</h4>
            ...
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={chartData} barGap={2}>
                ...
                <Line yAxisId="right" type="monotone" dataKey="到诊率" stroke="#F59E0B" strokeWidth={2.5} dot={{ r: 4, fill: '#F59E0B', strokeWidth: 2, stroke: '#fff' }} />
              </BarChart>
            </ResponsiveContainer>
          </div>

          <div className="content-card p-5">
            <h4 className="text-sm font-semibold text-neutral-800 mb-4">各项目4周汇总</h4>
            <div className="space-y-2">
              {treatmentSummary.map(item => (
                <div key={item.treatment} className="rounded-xl border border-neutral-100 overflow-hidden">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="w-16 text-sm font-medium text-neutral-800">{item.treatment}</span>
                          ...
                          return (
                            <div key={week} className="p-2 bg-neutral-50 rounded-lg">
                              ...
                              <p className={`text-xs font-medium mt-0.5 ${
                                (wd?.noShowRate || 0) > 10 ? 'text-danger-600' : 'text-success-600'
                              }`}>
                                爽约 {wd?.noShowRate || 0}%
                              </p>
                            </div>
